...
- P1のnative assetは5方式すべて制作を終えた。**`M3-04`（方式C）はPR #30がmergeされ受入済みである。**`pipeline/m3/c/` に layer / control cage / deformer / mask / draw order を方式C固有に定義し、外部asset入力は0件である。閉口時の口腔遮蔽と開口時の露出を対で検証し、`npm run m3:validate-c-asset` と23 testsを受入証跡とする。
- 残る4方式は **PR #35（D）/ #36（E）/ #32（B1）/ #31（B2）として提出済みでmerge待ち**である。**merge前なので `DONE` ではない。** 受入証拠が `main` にあるものだけを完了として扱う。
