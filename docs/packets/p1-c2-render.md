...
`nativeAssetVersions` が **3 / 承認上限 2**（[p1-d0](p1-d0-owner-decisions.md) 決定④）。
**`C-KF-004` が実フレームで欠陥として現れても、この packet では直せない。**
観測し、記録し、**止めて報告する**。決定④（バグ修正を回数に数えるか）が未決のまま欠陥が出た場合は特にそうする。

### ネットワーク接触面がゼロかもしれない

決定⑤ により C は自前ラスタライザで描画するため、Chrome も Blender も使わない。
**したがって enforcement 証跡が不要である**（決定① の表を参照）。preflight がそれを正しく扱うことを確認する。
...
| enforcement 証跡 | **不要**（決定⑤ により Chrome も Blender も使わない） |
| owner 判断 | 決定④（C のバグ修正を回数に数えるか）が望ましい。決定⑤ は Orbit 決定済み |
