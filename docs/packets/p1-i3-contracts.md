...
- framing 契約の具体値（ランドマーク位置と許容差）は**この packet が起案する**。[p1-d0](p1-d0-owner-decisions.md) 決定⑥ に依存する
...
| merge 済み PR | なし |
| owner 判断 | **決定②（各テストの長さ）のみ。** 未決なら仮値を置き `status` に明記して報告する。決定⑥（共通 framing）は Orbit 決定済み |
...
- framing ランドマークが、ある方式で許容差内に収まらないと判明した場合（**設計の見直しが要る。止めて報告する**）
- 総フレーム数が決まらないと契約が書けない場合（**決定② が必要**）。仮値で進み明記して報告する
