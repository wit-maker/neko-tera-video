...
- debug の構成は決定⑦ で確定している。層1（方式非依存）は**横並びで比較する**。層2（機構固有）は**方式ごとの個別証拠**とし、横並びには出さない
...
| merge 済み PR | なし |
| owner 判断 | なし。**決定⑦ は Orbit が決定済み** — 層2 は個別証拠として生成し、**横並びには出さない** |
...
- 層2 が個別証拠として置かれ、横並びに出ていないことの確認方法
