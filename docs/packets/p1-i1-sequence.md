...
- **sequence の中身は Orbit が起案する。** ただし render 前に、mi3san へ**平易な言葉で「何を見せるテストか」を提示して確認を得る**（決定⑧）。JSON への署名を求めるのではなく、「見たいものが入っているか」を確認してもらう
- artifact の総フレーム数は [p1-d0](p1-d0-owner-decisions.md) **決定② に依存する**。決定前は 619 フレーム1本を仮に採り、決定で変わりうることを JSON の `status` に明記する
...
| merge 済み PR | なし |
| owner 判断 | 決定②（各テストの長さ）が未定でも着手可。**render 前に、平易な要約で mi3san の確認を得る**（決定⑧） |
...
- 各テストの長さ（[p1-d0](p1-d0-owner-decisions.md) 決定②）が必要になった場合。仮値で進み、JSON に明記して報告する
...
- 総フレーム数が決定②（各テストの長さ）に依存すること
- **mi3san へ提示する平易な要約**（各テストが何を見せるものかを、内部用語なしで説明したもの）。render 前にこれで確認を得る
